...
import styles from "./network/tickets.module.css";
...
export default function TicketsPage() {
  ...
  const handleDeleteTicket = async (ticketId: string, e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(`/api/network/tickets/${ticketId}`, { method: "DELETE" });
  ...
  const handleQuickComplete = async (ticketId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const res = await fetch(`/api/network/tickets/${ticketId}`, {
        method: "PATCH",
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/network/tickets?${params.toString()}`);
    ...
    <div className={clsx("page-container", "animate-fade-in")}>
      <header className={styles.pageHeader}>
        ...
        <div className={styles.headerActions}>
          <Link href="/network/tickets/new" className={pageStyles.btnPrimary}>
          ...
          <table className={pageStyles.table}>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={ticket.id}>
                  <td><Link href={`/network/tickets/${ticket.id}`} className={pageStyles.idLink}>#{ticket.ticketNumber.slice(-4)}</Link></td>
                  <td>
                  ...
                  <td className={styles.actionWrapper}>
                    ...
                    {openMenuId === ticket.id && (
                      <div className={styles.dropdownMenu} onClick={(e) => e.stopPropagation()}>
                        <Link href={`/network/tickets/${ticket.id}`} className={styles.dropdownItem}>
          ...
          {isLoading ? (
            ...
            tickets.map((ticket) => (
              <div key={ticket.id} className={styles.mobileCard}>
                  ...
                  <div className={styles.cardAction}>
                    ...
                    {openMenuId === ticket.id && (
                      <div className={styles.dropdownMenu} style={{ right: 0, top: 32 }} onClick={(e) => e.stopPropagation()}>
                        <Link href={`/network/tickets/${ticket.id}`} className={styles.dropdownItem}>Просмотр</Link>
                ...
                <Link href={`/network/tickets/${ticket.id}`} className={styles.cardBodyLink}>
